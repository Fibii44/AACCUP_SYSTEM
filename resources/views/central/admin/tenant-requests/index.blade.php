<x-layouts.app>
    <div class="p-6">
        <h1 class="text-2xl font-semibold mb-6">Department Registration Requests</h1>
        ...
        <div class="bg-white shadow rounded-lg overflow-hidden">
                        ...
                        @foreach($pendingRequests as $request)
                            <tr>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm font-medium text-gray-900">{{ $request->department_name }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm text-gray-500">{{ $request->email }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm text-gray-500">{{ $request->domain }}.{{ config('app.domain') }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                    <div class="flex space-x-2">
                                        <a href="{{ route('admin.tenant-requests.show', $request) }}" class="text-indigo-600 hover:text-indigo-900">View Details</a>
                                    </div>
                                </td>
                            </tr>
                        @endforeach
            ...
            @else
                <div class="p-6 text-center">
                    ...
                    <p class="text-gray-500 mb-4">No pending department registration requests</p>
                </div>
            @endif
        </div>
    </div>
